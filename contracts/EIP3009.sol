...
abstract contract EIP3009 is EIP712 {
    ...
    /**
     * @notice Burns tokens after verification
    ...
    /**
     * @notice Mints tokens after verification
    ...
    function _cancelAuthorization(
        ...
        if (_authorizationStates[authorizer][nonce])
            revert EIP3009_AuthorizationUsed();
    ...
    function _transferWithAuthorization(
        ...
        if (block.timestamp < validAfter)
            revert EIP3009_AuthorizationNotYetValid();
        if (block.timestamp > validBefore)
            revert EIP3009_AuthorizationExpired();
    ...
    function _requireValidSignature(
        ...
        if (signer != ECDSA.recover(_hashTypedDataV4(dataHash), signature))
            revert EIP3009_InvalidSignature();
    }
